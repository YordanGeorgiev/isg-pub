package DocPub::View::DocSrcCodeBuilder ; 
   ...
   # -----------------------------------------------------------------------------
	sub doBuildControl {
		...
		$content				= $row->{'SrcCode'} ; 
		$content				= DocPub::View::HtmlHandler::convertHtmlEntities ( $content ) ;
		my $file_type		= $row->{'FileType'} ; 

		my $tab_inx_seq	= $row->{'SeqId'} ;  
		$paragraph			.= '<div class="edit_area src_code" ' ; 
		#$paragraph			.= 'tabindex="' . $tab_inx_seq .  '" style="min-height: 150px;"' ; 
		$paragraph			.= 'style="min-height: 50vh"' ; 
		$paragraph			.= 'id="dp_SrcCode' . '-' . $id . '"> ' ;  
		$file_type 			 = 'sh' unless ( $file_type ) ; 
		$paragraph 			.= '<pre tabindex="0" class="prettyprint lang-' . $file_type . '">' ; 
