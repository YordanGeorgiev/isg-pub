package DocPub::View::PdfDocBuilder ; 
   ...
   # -----------------------------------------------------------------------------
	sub doBuildControl {
		...
		$objTitleControlBuilder 		= $objTitleControlFactory->doInstantiate ( 'pdf_doc_title' ) ;
		$objPrgrphControlFactory 		= 'DocPub::View::ControlFactory'->new( \$objController);
		$objPrgrphControlBuilder 		= $objPrgrphControlFactory->doInstantiate ( 'pdf_doc_prgrph' ) ;
		$objSrcCodeControlFactory 		= 'DocPub::View::ControlFactory'->new( \$objController);
		$objSrcCodeControlBuilder 		= $objSrcCodeControlFactory->doInstantiate ( 'pdf_src_code' ) ;
		...
		foreach my $row ( @$rs )  {
			...
			foreach my $num ( sort ( keys ( %$rs_meta ) )  ) {
				...
				if ( $src_code ne ' ' && $src_code ne '_' && $src_code ne ''
					&& $column_name eq 'SrcCode' && $row->{'Level'} != 0 && $hdrs_only != 1 ) {
...
#			}
   ...
   # -----------------------------------------------------------------------------
	sub doResetAllSubordinateLevels {
		...
		#debug print "PdfDocBuilder:: doResetAllSubordinateLevels" ; 
