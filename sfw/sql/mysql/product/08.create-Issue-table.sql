...
--	 , `LogicalOrder`		varchar(30) 	NOT NULL
	...
	DROP TRIGGER IF EXISTS `trgOnInsertIssueUpdateTime` ;
	
	
	CREATE TRIGGER `trgOnInsertIssueUpdateTime` BEFORE INSERT ON  `Issue` 
	FOR EACH ROW 
	...
	DROP TRIGGER IF EXISTS `trgOnUpdateIssueUpdateTime` ;
	CREATE TRIGGER `trgOnUpdateIssueUpdateTime` BEFORE UPDATE ON  `Issue` 
		FOR EACH ROW 
		BEGIN 
			-- update the update time of the Issue
			SET NEW.UpdateTime = NOW() ; 
		END ;
