	...
	DROP TRIGGER IF EXISTS `trgOnInsertDevOpsUpdateTime` ;
	
	
	CREATE TRIGGER `trgOnInsertDevOpsUpdateTime` BEFORE INSERT ON  `DevOps` 
	FOR EACH ROW 
	...
	DROP TRIGGER IF EXISTS `trgOnUpdateDevOpsUpdateTime` ;
	CREATE TRIGGER `trgOnUpdateDevOpsUpdateTime` BEFORE UPDATE ON  `DevOps` 
		FOR EACH ROW 
		BEGIN 
			-- update the update time of the DevOps
			SET NEW.UpdateTime = NOW() ; 
		END ;
