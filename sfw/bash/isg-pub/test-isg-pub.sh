#!/bin/bash 
# file: sfw/bash/test-isg-pub.sh docs at the eof file
...
doRunTests(){
	...
	while read -r action ; do (
		...
		done < <(find sfw/bash/isg-pub/tests -type f -name '*.sh')

		doLog "INFO STOP :: testing action: \"$action\""
	);
	done < <(cat $product_version_dir/sfw/bash/isg-pub/tests/run-isg-pub-tests.lst)
