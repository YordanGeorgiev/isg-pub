...
#v0.2.1
#------------------------------------------------------------------------------
# creates the partial package based on the meta/isg-pub.deploy files
#------------------------------------------------------------------------------
doCreateDeploymentPackage(){
	...
	#define default vars
	test -z $include_file         && \
		include_file="$product_version_dir/meta/.deploy.$component_name"
	...
	xargs -0 -I "{}" zip -MM $zip_file "$org_name/$component_name/$environment_name/{}"
