...
doGenerateActionFiles(){
	...
	while read -r act ; do (
		...
		doLog "DEBUG START :: checking action: $act"
			...
			while read -d" " deliverable_type ; do (
				...
				doLog " INFO deliverable_doc_file: $deliverable_doc_file"
				doLog " INFO deliverable_code_file: $deliverable_code_file"
				...
				if [ $doc_file_exists -eq 0 ];then
					...
					cp -v docs/txt/isg-pub/tmpl/%act%.%deliverable_type%.txt "$deliverable_doc_file"
	...
	done< <(cat sfw/bash/$wrap_name/tests/all-isg-pub-tests.lst)
