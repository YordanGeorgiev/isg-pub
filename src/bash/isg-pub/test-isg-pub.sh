#!/bin/bash 
# file: src/bash/isg-pub/test-isg-pub.sh
      ...
      doLogTestRunEntry 'START'
		while read -r test_file ; do (
            ...
            then
               ...
               $function_name
               doLog "INFO test-isg-pub loop exit_code: $exit_code" 
               doLogTestRunEntry 'INFO' ' '$(date "+%H:%M:%S")
		...
		done < <(find src/bash/isg-pub/tests -type f -name '*.sh')

	);
	done < <(cat $product_instance_dir/src/bash/isg-pub/tests/run-isg-pub-tests.lst)
...
doLog(){
   ...
   test -t 1 && echo " [$type_of_msg] `date "+%Y.%m.%d-%H:%M:%S"` [isg-pub][@$host_name] [$$] $msg "
...
#----------------------------------------------------------
# Purpose:
# a simplistic tester for the simplistic bash stub isg-puber
#----------------------------------------------------------
#
#----------------------------------------------------------
...
#----------------------------------------------------------
#
# eof file: src/bash/isg-pub/test-isg-pub.sh
